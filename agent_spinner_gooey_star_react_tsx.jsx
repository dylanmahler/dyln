...
// Lightweight CSS mesh: 3 moving radial-gradients (blue, green, soft yellow)
const gradientKeyframes = `
@keyframes meshShift {
  0%   { background-position: 20% 30%, 80% 25%, 50% 75%; }
  50%  { background-position: 35% 20%, 70% 40%, 55% 60%; }
  100% { background-position: 20% 30%, 80% 25%, 50% 75%; }
}`;
  ...
  return (
    <div
      ...
      style={{
        ...
        height: "100%",
        display: "block",
        backgroundImage: `radial-gradient(40% 40% at 30% 30%, rgba(80,160,255,0.9), rgba(80,160,255,0) 60%),
                          radial-gradient(45% 45% at 70% 40%, rgba(80,230,170,0.85), rgba(80,230,170,0) 58%),
                          radial-gradient(55% 55% at 50% 75%, rgba(255,245,190,0.8), rgba(255,245,190,0) 60%)`,
...
export default function AgentMorphSpinner({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const svg = el.closest("svg");
      if (svg) svg.style.transform = `rotate(${rotateRef.current * (1 - t)}deg)`;
      if (t < 1) requestAnimationFrame(tick); else rotateRef.current = 0;
    };
  ...
  return (
    <div style={{ display: "grid", placeItems: "center", gap: 12 }}>
      ...
      >
        <defs>
          <clipPath id="agent-clip">
            {/* This path is the morphing silhouette */}
            <path ref={pathRef} d={AGENT_PATHS[0]} />
          </clipPath>
        </defs>

        {/* IDLE: simple gray fill */}
        {!thinking && (
          <path d={AGENT_PATHS[0]} fill={"#696365"} />
        )}

        {/* THINKING: lightweight CSS mesh clipped to the path */}
        {thinking && (
          <foreignObject x="0" y="0" width="24" height="24" clipPath="url(#agent-clip)">
            <MeshDiv />
          </foreignObject>
        )}
      </svg>
      ...
      <button
        ...
        style={{
          ...
          background: thinking ? "#696365" : "#fff",
          color: thinking ? "#fff" : "#696365",
          fontSize: 14,
          cursor: "pointer",
        }}
      >
